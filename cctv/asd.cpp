...
// 필독! -> 컨트롤러 포트 번호를 맞게 입력하고 첨부한 data 폴더를 C드라이브에 꼭꼭꼭 넣어야 합니다!

using namespace cv;
using namespace std;

int face_detect(int, char**); // 얼굴 찾는 함수
void fdad(IplImage *, CSerialComm serialComm); // 얼굴 서클그려주는 함수 + 바이트 시리얼 통신 함수
...
int face_detect(int argc, char **argv) {
	...
	if (!arduino_Port.connect("COM5")) {  // 아두이노 포트 확인 제 컴퓨터의 경우 COM3 포트
		/*
		(중요!) 위에 포트 번호를 장치 관리자에서 검색해서 컨트롤러 해당하는 포트로 넣어야 합니다!
		*/
	...
	if (argc > 1 && strncmp(argv[1], "cascade -> ", optlen) == 0) {
		...
		/*
		(중요!) C 드라이브에 첨부한 data 파일을 그대로 복사 해야 얼굴을 인식할 수 있습니다!!!!
		*/
		input_name = argc > 1 ? argv[1] : 0;
	}


	if (!(cascade = (CvHaarClassifierCascade *)cvLoad(XML_name, 0, 0, 0))) { // XML 파일 로드
